...
void bgs::new_fgmask(cv::Mat frame)
{
		...
		double mean; // initialize a mean value
		double sigma; // initialize a sigma value
		int std = 0.5; // standard deviation value
        double alpha_gaussian = 0.05; // alpha value


		int pixvalue=0;  // will save a frame current pixel value
        int maskvalue = 0; // set mask value in the loop

		for(int row = 0;row<frame.rows;row++)
		{
			for(int column = 0;column<frame.cols;column++)
			{
				pixvalue = frame.at<uchar>(row, column);
				mean = _mean.at<uchar>(row, column);
				sigma = sqrt(_sigma.at<uchar>(row, column));

				if(abs(pixvalue-mean) > std*sigma)
				{
					maskvalue = 1;
				}
				else
				...
				if(0 == maskvalue)
				{
					_mean.at<uchar>(row, column)=alpha_gaussian*pixvalue+(1-alpha_gaussian)*mean;
				}
					_sigma.at<uchar>(row, column)=alpha_gaussian*pow((pixvalue-mean),2)+(1-alpha_gaussian)*_sigma.at<uchar>(row, column);

			}
		}

		_fgmask = mask; //initialize fgmask
}
